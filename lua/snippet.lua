...
function M.snippet_handler(paths, ft, descs)
  ...
  if client then
    ...
    if false then vim.notify("Stopped previous sn_ls client with " .. client.id, vim.log.levels.INFO) end
  end
  ...
  vim.defer_fn(function()
    if ft ~= "" then -- do not run on empty filetype causes weird behavior
      ...
      if false then vim.notify("Started new sn_ls client with " .. client_id .. " for " .. ft, vim.log.levels.INFO) end
    end
  end, 750)
end
